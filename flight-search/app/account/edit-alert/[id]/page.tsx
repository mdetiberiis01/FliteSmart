...
import { getUserAlerts, UserAlert } from '@/lib/supabase/user-alerts';
...
export default function EditAlertPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const alert = alerts.find((a: UserAlert) => a.id === id);
      if (!alert) { router.push('/account'); return; }

      setOrigin(alert.origin);
      setOriginName(alert.origin_name ?? '');
      setDestination(alert.destination);
      setMaxPrice(String(alert.max_price));
      if (alert.flexibility) setFlexibility(alert.flexibility as Flexibility);
      if (alert.custom_date_start) setCustomDateStart(alert.custom_date_start);
      if (alert.custom_date_end) setCustomDateEnd(alert.custom_date_end);
      if (alert.trip_days) setTripDays(alert.trip_days);
    }
    load();
  }, [router, id]);
      ...
      <section className="bg-gradient-to-b from-sky-50 via-sky-50/40 to-white dark:from-slate-900 dark:via-slate-900/40 dark:to-[#0a0a0a] pt-16 pb-24 px-4 flex-1">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            <h1 className="text-4xl font-bold text-black dark:text-white mb-3 tracking-tight">Edit alert</h1>
            <p className="text-black/50 dark:text-white/50 text-lg">
              Update your route or price target.
            </p>
          </div>
